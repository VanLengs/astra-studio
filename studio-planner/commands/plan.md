...
1. **event-storm** — Multi-role brainstorming for `$ARGUMENTS`.
   - Invokes: `persona-insight` → persona cards
   - Invokes: `journey-map` → user journey maps
   - Invokes: `process-flow` → business process diagrams
   - Also produces: event list, hotspot ranking

2. **domain-model** — Analyze event storm output to identify plugin boundaries.
   - Invokes: `domain-canvas` → domain boundary map
   - Invokes: `behavior-matrix` → actor/action/event cross-reference
   - Invokes: `opportunity-brief` → prioritized opportunity assessment
...
After each pipeline step, pause and present results to the user for validation before proceeding. The user may also invoke any artifact skill independently at any time (e.g., `/studio-planner:journey-map` for a standalone journey map).
