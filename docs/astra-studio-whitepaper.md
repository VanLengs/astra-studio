...
graph TD
    ...
    subgraph "阶段一：发现"
        ...
        ES --> |"调用"| PI[persona-insight → 画像卡片]
        ES --> |"调用"| JM[journey-map → 旅程地图]
        ES --> |"调用"| PF[process-flow → 流程图]
        ES --> |"产出"| HS[热点排名]
    end
    ...
    subgraph "阶段二：建模"
        DM[domain-model] --> |"调用"| DC[domain-canvas → 领域画布]
        DM --> |"调用"| BM[behavior-matrix → 行为矩阵]
        DM --> |"调用"| OB[opportunity-brief → 机会评估]
        DM --> |"产出"| PC[插件候选]
    end
